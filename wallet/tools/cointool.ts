...
namespace EasyCheers.tools {
    export class CoinTool {
        ...
        static readonly id_NNC: Neo.Uint160 = Neo.Uint160.parse('fc732edee1efdf968c23c20a9628eaa5a6ccb934');
        static readonly dapp_nnc: Neo.Uint160 = Neo.Uint160.parse("fc732edee1efdf968c23c20a9628eaa5a6ccb934");
        static readonly id_ECT: Neo.Uint160 = Neo.Uint160.parse('e41bf464feedae9409f19671a773860ceafcd300');
        ...
        static makeTran(utxos: { [id: string]: UTXO[] }, targetaddr: string, assetid: string, sendcount: Neo.Fixed8): Result
        {
            ...
            if (gasutxos && CoinTool.id_GAS == assetid)   //如果转账的金额是gas
            {
        ...
        static async claimGas()
        {
            ...
            let claimsstr = await tools.WWW.api_getclaimgas(address, 0);
         ...
         * @param amount 转账数额
         */
        static async nep5Transaction(address: string, tatgeraddr: string, asset: string, amount: number)
        {
            console.log(address);
        ...
        static async getavailableutxos(count: number): Promise<{ [id: string]: UTXO[] }>
        {
            ...
            let utxos = await tools.WWW.getavailableutxos(currentaddr, count);
            var assets = {};        //对utxo进行归类，并且将count由string转换成 Neo.Fixed8
         ...
         * @param odds 堵住
         */
        static async bet(contractHash: string, address: string, max: number, odds: number)
        {
            console.log(address);
            ...
            sb.Emit(ThinNeo.OpCode.DROP);
            //塞值
            sb.EmitParamJson(["(address)" + address, "(integer)" + max, "(integer)" + odds]);//第二个参数是个数组
